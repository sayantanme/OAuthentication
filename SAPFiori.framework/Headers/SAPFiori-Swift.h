...
/// <h2>Usage</h2>
/// <code>FUINavigationBar</code> has a <code>FUIBannerMessageView</code> prepared for you. Usually you don’t need to create a instance of it.
/// <h3>Setup</h3>
...
@property (nonatomic) CGFloat _splitPercent;
...
/// }
///
/// // While the FUIModalCheckoutViewController is shown, you might want change the display state according to 
...
///
/// \endcode<h2>Note:</h2>
/// <code>FUIObjectHeader</code> added to <code>UITableView</code> as tableHeaderView is compatable with <code>UIRefreshControl</code> added to the same table view.  Just note that if an <code>UIRefreshControl</code> object is set after adding an <code>FUIObjectHeader</code> object as tableHeaderView,  the <code>UIRefreshControl</code> object must be moved to the front in the table view; otherwise, the <code>UIRefreshControl</code> object wouldn’t be visible.
/// <ul>
///   <li>
///     When <code>FUIObjectHeader</code> is added to a table view in a storyboard and <code>UIRefreshControl</code> is added later on in a table view controller class’ <code>viewDidLoad</code> function, bring the refreshControl to the front:
///   </li>
/// </ul>
/// \code
///
...
/// apps or music.
/// Different display states control the visual apprearance of the control and the options the user has to interact with the control.
/// <h2>Initialization</h2>
/// <h3>Programmatically:</h3>
/// \code
...
@interface FUIResizingCollectionView (SWIFT_EXTENSION(SAPFiori)) <UICollectionViewDelegateFlowLayout>
/// :nodoc:
...
@interface FUIResizingCollectionView (SWIFT_EXTENSION(SAPFiori)) <UIScrollViewDelegate>
/// :nodoc:
...
@interface FUIResizingCollectionView (SWIFT_EXTENSION(SAPFiori)) <UICollectionViewDelegate>
/// :nodoc:
...
/// Reuse identifier
/// @return String a reuse identifier that describes FUITableViewHeaderFooterView
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSString * _Nonnull reuseIdentifier;)
+ (NSString * _Nonnull)reuseIdentifier SWIFT_WARN_UNUSED_RESULT;
/// :nodoc:
- (nullable instancetype)initWithCoder:(NSCoder * _Nonnull)aDecoder OBJC_DESIGNATED_INITIALIZER;
